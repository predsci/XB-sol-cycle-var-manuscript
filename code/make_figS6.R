...
for (jj in 1:ncrit) {
	...
	for (ii in 1:ncycle) {
	  ...
	  ii1m = ii - 1
		...
		clauset.list <- clauset(data_estimates, boot = boot, bootShow = bootShow, xcrit = inp.xcrit,
		                        xmin = xmin_arr[ii], xaxmin = NULL, yaxmax = yaxmax, dt = dt, tSpan = tSpan, continous = continous)
...
fname = paste0('../plots/figure_S6.png')
ggsave(fname, mp, width = 20, height = 12, units = 'in')
cat('\nSaved Plot at: ', fname,'\n')

fname = paste0('../plots/data_S6.csv')
