...
source('../../XB-sol-cycle-var-manuscript/code/util.R')

mypath = '~/Dropbox/NAPR08/oulu_data/XB-sol-cycle-var-manuscript'
...
for (ii in 1:(ncycle-1)) {
  ...
  xmin_arr[ii1] = threshold

  cat("For Cycle ", ii, " KS Estimate for xmin: ", xmin_arr[ii1], '\n')
}
...
for (jj in 1:ncrit) {
	...
	for (ii in 1:ncycle) {
	  ...
	  ii1m = ii - 1
		...
		##
		## Clauset Analysis -  with the subjective threhold
		##

		clauset.list <- clauset(data_estimates, boot = boot, bootShow = bootShow, xcrit = inp.xcrit, xmin = xmin_arr[ii], xaxmin = NULL, yaxmax = yaxmax, dt = dt, tSpan = tSpan, continous = continous)
...
fname = paste0('../plots/figure_S7.png')
ggsave(fname, pl.all, width = 12, height = 7, units = 'in')

cat('\nSaved Plot at: ', fname,'\n')

fname = paste0('../plots/data_S7.csv')
