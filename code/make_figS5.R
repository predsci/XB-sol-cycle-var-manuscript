...
pdf(file = 'threshold_plot.pdf', width = 12, height = 30)
par(mfrow = c(ncycle,2), mar=c(2,4,2,2))
...
for (ii in 1:(ncycle-1)) {
  ...
  #mrlplot(-data$value, xlim = c(110, 200))
  rmin = 110
  rmax = round(max(-data$value)*0.7)
  nint = 40
  rmax = min(rmax, 200)
  threshrange.plot(-data$value, r = c(rmin,rmax), type = 'GP', nint = nint, set.panels = FALSE)
}
...
for (ii in 1:(ncycle-1)) {
  ...
  #abline(v = ks_thresh[ii], col = 'red', lty = 3)
}
...
# do the GP fits with the following subjective thresholds

thresh_subjective = c(130, 130, 130, 130, 140, 120, 120, 115, 120)

pdf(file = 'fevd_plot.pdf', width = 12, height = 8)
...
for (ii in 1:(ncycle-1)) {
  ...
  tspan_days = as.numeric(date_end - date_start)
  tspan_years = tspan_days / 365
  events_per_year = round(dim(data)[1]/tspan_years,2)
  text_time_units = paste0(events_per_year,'/year')
fitFC <- fevd(-data$value, threshold = thresh_subjective[ii], type = "GP", time.units = text_time_units)
plot(fitFC)
#print(fitFC)
}
dev.off()
...
fname = paste0('../plots/figure_S5.png')
